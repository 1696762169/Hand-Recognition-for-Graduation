...
path_to_db = 'D:/Python/dataset/RHD/'
...
def write_to_binary(file_handle, image: np.ndarray, mask: np.ndarray, kp_coord_xyz, kp_coord_uv, kp_visible, K_mat):
    ...
    # 4. write image
    flatten_image = image.flatten().astype(np.uint8)
    file_handle.write(flatten_image.tobytes())
    ...
    # 5. write mask
    flatten_mask = mask.flatten().astype(np.uint8)
    file_handle.write(flatten_mask.tobytes())
...
with open(file_name_out, 'wb') as fo:
    ...
    for sample_id, anno in anno_all.items():
        ...
        mask = cv2.imread(os.path.join(path_to_db, set, 'mask', '%.5d.png' % sample_id), cv2.IMREAD_GRAYSCALE)
