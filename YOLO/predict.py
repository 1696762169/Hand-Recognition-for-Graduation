...
def load_model(model_path: str, cfg: str, data_cfg: str, img_size: int) -> nn.Module:
...
def predict_image(
    ...
    """
    预测图片的手部位置
    :param img: 图片数据 (H, W, C)
    :param model: 模型
    :param img_size: 模型输入图像尺寸
    :param conf_thres: 置信度阈值
    :param nms_thres: nms 阈值
    :return: 手部置信度和位置信息 (置信度, (x1, y1, x2, y2))
    """
    use_cuda = torch.cuda.is_available()
    device = select_device() # 运行硬件选择
    ...
    ratio = float(img_size) / max(original_shape[:2])  # ratio  = old / new
    new_shape = (round(original_shape[0] * ratio), round(original_shape[1] * ratio))
    dh = (img_size - new_shape[0]) / 2  # height padding
    dw = (img_size - new_shape[1]) / 2  # width padding
    top, bottom = round(dh - 0.1), round(dh + 0.1)
    left, right = round(dw - 0.1), round(dw + 0.1)

    img = F.interpolate(img.permute(2, 0, 1).unsqueeze(0), size=new_shape, mode='bilinear', align_corners=False)
    img = F.pad(img, (left, right, top, bottom), value=0.5)  # pad with 0.5
    ...
    # t2 = time.time()
    # print("inference time:", t2-t1)
    detections = non_max_suppression(pred.detach(), conf_thres, nms_thres)[0] # nms
    if use_cuda:
        torch.cuda.synchronize()
    # t3 = time.time()
    # print("get res time:", t3-t2)
    ...
    # 未检测到手部的情况
    if detections is None or len(detections) == 0:
        return (0.0, (0, 0, original_shape[1], original_shape[0]))
    #     cv2.namedWindow('image',0)
    #     cv2.imshow("image", im0)
    #     key = cv2.waitKey(1)
    #     if key == 27:
    ...
    # Rescale boxes from 416 to true image size
    detections[:, :4] = scale_coords(img_size, detections[:, :4], original_shape).round()
    # result = []
    # for res in detections:
    #     result.append((classes[int(res[-1])], float(res[4]), [int(res[0]), int(res[1]), int(res[2]), int(res[3])]))
    # if use_cuda:
    #     torch.cuda.synchronize()
    ret = detections[0]
    return (float(ret[4]), (int(ret[0]), int(ret[1]), int(ret[2]), int(ret[3])))


if __name__ == '__main__':

    voc_config = 'cfg/hand.data' # 模型相关配置文件
    model_path = './weights/hand_416_good.pt' # 检测模型路径
    model_cfg = 'yolo' # yolo / yolo-tiny 模型结构
    video_path = "./videos/test2.avi" # 测试视频
